...
public class Car {
    ...
    /* 시동이 걸려있는 상태에서는 할일이 없고 시동이 걸려있지 않은 경우 시동을 건다.*/
    public void startUp(){
        if(isOn){
        ...
        System.out.println("시동을 걸었습니다. 이제 출발한 준비가 완료되었습니다.");
    }
    ...
    public void stop(){
        if (isOn){
            ...
            System.out.println("차의 시동이 걸려있지 않습니다. 시동 먼저 걸어주세요.");
        }
    }
    public void trunOff(){
        if(isOn){
            if (speed > 0){
                System.out.println("달리고 상태에서 시동을 끌수 없습니다. 차를 우선 멈춰주세요.");
            }else {
                this.isOn = false;
